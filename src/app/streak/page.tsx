...
export default function StreakCelebrationPage() {
  ...
  return (
    <ScreenFrame>
      <section className="flex h-dvh min-h-dvh flex-col bg-vocali-teal-deep px-6 pb-4 pt-4 text-white sm:h-[860px] sm:min-h-[860px] sm:pb-6 sm:pt-6">
        <header className="flex h-9 shrink-0 items-center justify-center overflow-visible">
          <div className="scale-[0.72] opacity-88 [&_*]:text-white sm:scale-90">
            <VocaliLogo size="sm" />
          </div>
        </header>

        <div className="flex min-h-0 flex-1 flex-col items-center justify-center py-2 text-center sm:py-4">
          <div className="relative flex h-[clamp(8.6rem,25dvh,15rem)] w-[clamp(8.6rem,25dvh,15rem)] items-center justify-center rounded-full bg-white/8 shadow-[0_20px_46px_rgb(0_0_0/0.14)]">
            <div className="relative flex h-[clamp(7rem,20.5dvh,11.4rem)] w-[clamp(7rem,20.5dvh,11.4rem)] items-center justify-center rounded-full bg-vocali-orange text-white shadow-[0_20px_42px_rgb(255_122_26/0.30)]">
              <Flame
                className="h-[clamp(4.2rem,12dvh,7rem)] w-[clamp(4.2rem,12dvh,7rem)] fill-white"
                strokeWidth={2.5}
              />
            </div>
          </div>

          <h1 className="mt-4 text-[clamp(1.9rem,4.5dvh,3rem)] font-black leading-none tracking-[-0.05em] sm:mt-7">
            You&apos;re on a streak!
          </h1>

          <p className="mt-3 flex items-baseline justify-center gap-2.5 font-black leading-none tracking-[-0.07em] text-vocali-orange sm:mt-5 sm:gap-3">
          ...
          <p className="mt-2 text-sm font-bold text-white/72 sm:mt-4 sm:text-base">
            That&apos;s {streakStatus.count}{" "}
            {streakStatus.count === 1 ? "day" : "days"} of showing up.
          </p>

          <div className="mt-4 flex w-full max-w-xs justify-between rounded-[1rem] bg-white/7 px-3 py-2.5 sm:mt-7 sm:rounded-[1.2rem] sm:px-3.5 sm:py-3">
            {week.map((day, index) => {
